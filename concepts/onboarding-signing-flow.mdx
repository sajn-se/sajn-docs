...
When the document is sealed, sajn POSTs `DOCUMENT_COMPLETED` to your webhook. This is where you get the authoritative signal. The download endpoint returns a pre-signed URL — fetch it to get the bytes, then store them:
...
app.post('/webhooks/sajn', async (req, res) => {
  ...
  if (event === 'DOCUMENT_COMPLETED') {
    // 1. Ask for a pre-signed download URL (defaults to the signed document)
    const { downloadUrl } = await fetch(
      `https://app.sajn.se/api/v1/documents/${payload.id}/download?fileType=SIGNED`,
      { headers: { Authorization: `Bearer ${process.env.SAJN_API_KEY}` } }
    ).then(r => r.json());

    // 2. Fetch the file from the pre-signed URL
    const signedPdf = await fetch(downloadUrl).then(r => r.arrayBuffer());
