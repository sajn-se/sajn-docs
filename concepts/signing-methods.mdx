...
<Note>
  Availability is per-account. Not every scheme is enabled for every
  organization — if you send a `requiredSignature` your account doesn't have
  enabled, the API rejects it with `This eID scheme is not yet available`.
  Contact support to have a scheme enabled for your market.
</Note>
