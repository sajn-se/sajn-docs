...
```javascript
const batchUpdateFormFields = async (documentId, updates) => {
  const response = await fetch(
    `https://app.sajn.se/api/v1/documents/${documentId}/fields`,
    {
      ...
      body: JSON.stringify({
        // Each entry's fieldMeta is merged into the existing subfield,
        // preserving label, placeholder, description, etc. Max 50 per request.
  ...
  if (!response.ok) {
    throw new Error(`Failed to batch update fields: ${response.statusText}`);
  }

  // { success, updated, results: [{ key, success, error? }] }
  // success is true only when every update succeeded. Inspect results for
  // per-key failures (e.g. an unknown key) — valid updates still apply.
  return response.json();
};
...
const syncCrmToSajn = async (crmRecord) => {
  ...
  const result = await batchUpdateFormFields(documentId, [
  ...
  console.log(`Synced ${result.updated} fields to sajn document ${documentId}`);
  return result;
};
```
...
class SajnCrmIntegration {
  ...
  // Update a single field (TEXT content or one FORM subfield value)
  ...
  // Update many FORM subfield values atomically in one request
  async batchUpdateFormFields(documentId, updates) {
  ...
  async syncDealToDocument(deal, fieldMapping) {
    // TEXT fields aren't FORM subfields, so update them via the single-key
    // endpoint. (A document usually has just one or two TEXT fields.)
    if (deal.headerText) {
      await this.updateField(deal.sajnDocumentId, fieldMapping.header, deal.headerText, 'TEXT');
    }

    // Batch all FORM subfield values into a single atomic request — never
    // fire these as parallel single-key PATCHes, they would clobber each other.
    const formUpdates = [
      { key: fieldMapping.customerName, value: deal.customerName },
      { key: fieldMapping.customerEmail, value: deal.customerEmail },
      { key: fieldMapping.contractValue, value: deal.dealValue?.toString() },
      { key: fieldMapping.startDate, value: deal.startDate }
    ...
    return this.batchUpdateFormFields(deal.sajnDocumentId, formUpdates);
  }
