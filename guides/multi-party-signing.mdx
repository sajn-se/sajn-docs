...
Each individual signature fires `DOCUMENT_PARTY_SIGNED`, carrying the [Document + Party payload](/guides/webhooks#document--party-payload) — the whole document plus the party that just signed:
...
There is no `remainingSigners` count — derive it from `payload.document.parties` by counting the ones still `NOT_SIGNED`. Once every signing party is done, a separate `DOCUMENT_SIGNED` event fires with the plain [Document payload](/guides/webhooks#document-payload).
