...
Use the `fileType` query parameter to select which file the pre-signed URL points to:
...
curl -X GET "https://app.sajn.se/api/v1/documents/doc_123/download?fileType=ORIGINAL" \
  -H "Authorization: Bearer YOUR_API_KEY"

# Signing journal (audit trail)
curl -X GET "https://app.sajn.se/api/v1/documents/doc_123/download?fileType=JOURNAL" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

<Warning>
  The `downloadOriginalDocument` boolean parameter is **deprecated** — use `fileType=ORIGINAL` instead. When `fileType` is set, `downloadOriginalDocument` is ignored.
</Warning>
