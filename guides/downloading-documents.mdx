...
// sajn sends the secret you configured as a plain header — compare it, don't hash it
const secretMatches = (received) => {
...
app.post('/webhooks/sajn', async (req, res) => {
  if (!secretMatches(req.headers['x-sajn-secret'])) {
...
def webhook():
    # sajn sends the secret you configured as a plain header — compare it, don't hash it
    secret = request.headers.get('X-Sajn-Secret', '')
    if not hmac.compare_digest(secret, os.environ['WEBHOOK_SECRET']):
        abort(401)
