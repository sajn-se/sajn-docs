...
|---------|-----------|---------|
| `@sajn/embed-js` | Vanilla JS | 0.1.8 |
| `@sajn/embed-react` | React 18+ | 0.1.8 |
| `@sajn/embed-vue` | Vue 3.3+ | 0.1.8 |
...
| `signatureInputModes` | `('draw' \| 'type' \| 'upload')[]` | all | How the signer may create a drawn signature. Pass `['draw']` for hand-drawn only. The workspace allowlist takes precedence: this prop can only remove modes the workspace allows, never add one it has turned off. |
| `showDocumentId` | `boolean` | `true` | Show the document ID under the sign button |
...
Fired when the signer submits the form fields assigned to them. The document is not signed yet: the signing step opens once every signer with form fields has submitted.

**Payload:**

```typescript
interface VariablesSubmittedData {
  token: string;
  documentId: string;
  signerId: string;
}
...
<Note>
  **Access verification ("verify before opening") and embedding**

  If a document has an access-verification requirement (SMS, email, or BankID
  *before opening*), the embedded signer must clear that gate before any document
  content is loaded — the same as on the hosted signing page. Document content is
  never sent to the iframe until verification passes.

  Because embedded signing usually runs inside an application that has *already*
  authenticated the user, this extra step is often redundant. If you want the
  embedded flow to open directly, avoid enabling access verification on documents
  you intend to embed. (The per-signer **signing** verification — the gate at the
  moment of signing — always applies, regardless of embedding.)
</Note>

## Live Demo

See the embedding in action at [sajn-embedding-demo.vercel.app](https://sajn-embedding-demo.vercel.app/). The source code is available on [GitHub](https://github.com/sajn-se/sajn-embedding-demo).
