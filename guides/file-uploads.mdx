...
| `size` | Yes | File size in bytes (max `26214400` = 25 MB). |
...
- **`uploadUrl`** — presigned `PUT` URL, valid for **15 minutes**.
...
- **`PRIVATE`** (default) — access requires a fresh signed URL (valid for 15 minutes), retrieved via [`GET /api/v1/files/{id}`](/api/GET%20/api/v1/files/{id}).
...
3. **Upload promptly** — the presigned `uploadUrl` is only valid for 15 minutes.
