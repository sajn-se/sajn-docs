...
- **Image Support** - Hosted HTTP/HTTPS URLs (recommended) — see [Best practice: host images, don't inline them](#best-practice-host-images-dont-inline-them)
...
### Best practice: host images, don't inline them
...
To host an image, upload it once via the [presigned file-upload flow](/guides/file-uploads)
(or any CDN / public URL you control) and reference the resulting `https://` URL.
The same URL can be reused across every document and template — no per-document copies.

### Recommended: hosted HTTPS URL

```html
<img src="https://cdn.yourcompany.com/logo.png"
     alt="Company Logo"
     style="max-width: 200px; display: block; margin: 0 auto;" />
```

### Discouraged: inline base64 data URI
...
<Note>
HTML field content has a generous size ceiling (currently 100 MB) so existing
integrations are not broken. This limit will be lowered substantially once
consumers migrate to hosted image URLs — keeping content small (text + hosted
image references) keeps you well within any future limit.
</Note>
